import {projectInventory} from './inventory-projection.mjs';
...
export function forecastPlan(input,policy={},corrections=[],dates=[],asOf=new Date().toISOString().slice(0,10)){
 ...
 const offline=Array(56).fill(0),offlineCovered=Array(56).fill(false);(input.offlineDailySales||[]).slice(-56).forEach((n,i,a)=>offline[56-a.length+i]=Number(n)||0);(input.offlineCoverage||[]).slice(-56).forEach((v,i,a)=>offlineCovered[56-a.length+i]=Boolean(v));
 ...
 const avg=n=>{const values=copy.slice(-n).filter((_,i)=>covered[56-n+i]);const extra=offline.slice(-n).filter((_,i)=>offlineCovered[56-n+i]);return (values.length?values.reduce((s,x)=>s+x,0)/values.length:0)+(extra.length?Math.max(0,extra.reduce((s,x)=>s+x,0)/extra.length):0)};const avg7=avg(7),avg21=avg(21),avg56=avg(56);
 ...
 const variance=(values,coverage)=>{const observed=values.filter((_,i)=>coverage[i]);if(!observed.length)return 0;const mean=observed.reduce((a,b)=>a+b,0)/observed.length;return observed.reduce((sum,n)=>sum+(n-mean)**2,0)/observed.length;};
 // Estimate the two source variances on their own observed days, without treating unknown online days as zero.
 const std=Math.sqrt(variance(copy,covered)+variance(offline,offlineCovered));
 ...
 const projection=Array.isArray(input.arrivalLots)?projectInventory({currentQty:input.currentQty,dailyDemand:velocity,arrivals:input.arrivalLots,horizonDays:horizon,asOf}):null;
 ...
 else if(projection&&velocity>0&&projection.firstShortageDay!==null){alertLevel='critical';alertLabel='断货风险';alertReason=`按各批预计到仓日期计算，${projection.firstShortageDate}开始出现缺口，周期内最大缺口${projection.maxShortage}件`}
 ...
 else if(covered.filter((v,i)=>v||offlineCovered[i]).length<7){alertLevel='data';alertLabel='数据不足';alertReason='有效销量记录不足7天，建议仅供人工复核'}
 ...
 return {...legacy,dailySales:source.map((n,i)=>n+offline[i]),adjustedDailySales:copy.map((n,i)=>n+offline[i]),onlineDailySales:source,offlineDailySales:offline,projection,avg7,avg21,avg56,forecastDaily:velocity,forecastSales:velocity*30,forecastVersion:'7-21-56-v1',safetyStock:safety,targetQty:target,rawSuggestedProduction:raw,suggestedProduction:qty,unconstrainedQty:unconstrained,suggestedReplenishment:roundSupplyQuantity(Math.max(0,Math.ceil(velocity*(sea+review)+safety)-Number(input.currentQty||0)-Number(input.pendingShelf||0)-Number(input.seaInTransit||0)),moq,multiple),stockCoverDays:cover(input.currentQty),landedCoverDays:cover(Number(input.currentQty||0)+Number(input.pendingShelf||0)+Number(input.seaInTransit||0)),pipelineCoverDays:cover(legacy.inventoryPosition),stockoutDays,spikeDays,eventFactor,lifecycle,limits,policy,alertLevel,alertLabel,alertReason,projectedAtNextArrival:projection?projection.projectedAtNextArrival:Math.floor(Number(input.currentQty||0)-velocity*wait+seaQty),productionStartInDays:velocity>0?Math.floor(Math.max(0,legacy.inventoryPosition/velocity-p-sea)):null};
}
