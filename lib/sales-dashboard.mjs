...
export async function readSalesDashboard(db,actor,input={},today=reportToday()){
  ...
  const measures='SUM(qty) qty,SUM(CASE WHEN qty<>0 THEN 1 ELSE 0 END) qtyRecords,SUM(revenue) revenue,SUM(adCost) adCost,COUNT(*) records,SUM(CASE WHEN revenue IS NULL THEN 1 ELSE 0 END) revenueMissing,SUM(CASE WHEN adCost IS NULL THEN 1 ELSE 0 END) costMissing';
  ...
  const result=await db.batch([
    ...
    db.prepare(`SELECT business_date date,site,channel FROM sales_imports WHERE reversed_at IS NULL AND report_kind IN ('complete','zero') AND business_date BETWEEN ? AND ?${site?' AND site=?':''}${channel?' AND channel=?':''} GROUP BY business_date,site,channel`).bind(from,to,...(site?[site]:[]),...(channel?[channel]:[])),
  ]);
  const [currencies,roi,scopes,daily,skus,coverage]=result.map(r=>r.results||[]);
  const expectedScopes=(site?1:Object.keys(SITE_CURRENCIES).length)*(channel==='线下分销'?0:channel?1:2),coverageCount=new Map();
  for(const row of coverage)coverageCount.set(row.date,(coverageCount.get(row.date)||0)+1);
  const completeDates=new Set([...coverageCount].filter(([,n])=>expectedScopes>0&&n===expectedScopes).map(([date])=>date));
  const dailyRows=daily.map(r=>metrics({...r,salesComplete:completeDates.has(r.date)}));
  for(const date of completeDates)if(!dailyRows.some(r=>r.date===date))dailyRows.push(metrics({date,currency:site?SITE_CURRENCIES[site]:'UNKNOWN',qty:0,qtyRecords:0,records:0,revenue:null,adCost:null,revenueMissing:0,costMissing:0,salesComplete:true}));
  ...
  return {filters,totalQty,skuCount:skus.filter(r=>r.qty>0).length,currencies:currencyTotals,skuRanking:rankings,roiRanking:roiRows,scopes:scopes.map(metrics).sort((a,b)=>b.qty-a.qty),daily:dailyRows.sort((a,b)=>a.date.localeCompare(b.date)),generatedAt:new Date().toISOString()};
}
